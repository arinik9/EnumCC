...
import flow.FordFulkerson;
import myUtils.Edge;
import myUtils.EdgeWeightedGraph;
import myUtils.IndexMaxPQ;
import myUtils.UF;
 ...
 * source: https://algs4.cs.princeton.edu/64maxflow/GlobalMincut.java.html
 */
public class StoerWagnerGlobalMincut {
     ...
     */
    public StoerWagnerGlobalMincut(EdgeWeightedGraph G) {
        V = G.V();
        validate(G);
        minCut(G, 0);
        assert check(G);
    ...
    private CutPhase minCutPhase(EdgeWeightedGraph G, boolean[] marked, CutPhase cp) {
        ...
        while (!pq.isEmpty()) {
            ...
            cp.t = v;
            for (Edge e : G.adj(v)) {
                int w = e.getOtherVertexId(v);
                if (pq.contains(w)) pq.increaseKey(w, pq.keyOf(w) + e.getweight());
            }
        }
        cp.weight = 0.0;
        for (Edge e : G.adj(cp.t)) {
            cp.weight += e.getweight();
        }
        return cp;
    }
    ...
    private EdgeWeightedGraph contractEdge(EdgeWeightedGraph G, int s, int t) {
        ...
        for (int v = 0; v < G.V(); v++) {
            for (Edge e : G.adj(v)) {
                ...
                if (v < w) {
                    if (w == t)      H.addEdge(new Edge(v, s, e.getweight()));
                    else if (v == t) H.addEdge(new Edge(w, s, e.getweight()));
                    else             H.addEdge(new Edge(v, w, e.getweight()));
                }
            }
        }
        return H;
    ...
    private boolean check(EdgeWeightedGraph G) {
        ...
        for (int s = 0, t = 1; t < G.V(); t++) {
            ...
            for (Edge e : G.edges()) {
                int v = e.either(), w = e.getOtherVertexId(v);
                F.addEdge(new FlowEdge(v, w, e.getweight()));
                F.addEdge(new FlowEdge(w, v, e.getweight()));
            }
