 ...
 *  and flow. Note that the resulting network is directed. If one works on undirected network,
 *  it is sufficient to add the inserting edge twice, but in opposite way, like (from,to) and (to, from).
 *  <p>
...
//    }
    ...
    /**
     * Initializes the flow edges
     * 
     */
    public void initFlowEdges(double capacity_) {
    	for (int v = 0; v < V; v++)
            for (FlowEdge e : adj(v)) {
            	e.setFlow(0.0);
            	e.setCapacity(capacity_);
            }
    }
